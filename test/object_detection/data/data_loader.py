...
def partition_data(data_path, partition, n_nets):
    if os.path.isfile(data_path):
        with open(data_path) as f:
            data = f.readlines()
        n_data = len(data)
        label_path = data_path.replace("images", "labels")  # Adjust accordingly if needed
    else:
        print(data_path)
        n_data = len(os.listdir(data_path))
        label_path = os.path.join(data_path.replace("images", "labels"))  # Adjust accordingly if needed
...
def load_partition_data_coco(args, hyp, model):
    ...
    # print(os.path.join(path,train_path))
    ...
    #         /home/siu856522160/major/test/object_detection/model/yolov5/datasets/coco128
    # /home/siu856522160/major/test/object_detection/model/yolov5/datasets/coco128/images/train2017
    # images/train2017
    # Traceback (most recent call last):
    #   File "/home/siu856522160/major/test/object_detection/main_fedml_object_detection.py", line 175, in <module>
    #     model, dataset, _, args = init_yolo(args=args, device="cuda:0")  # Pass appropriate args and device
    #   File "/home/siu856522160/major/test/object_detection/model/init_yolo.py", line 195, in init_yolo
    #     dataset = load_partition_data_coco(args, hyp, model)
    #   File "/home/siu856522160/major/test/object_detection/data/data_loader.py", line 870, in load_partition_data_coco
    #     net_dataidx_map = partition_data(
    #   File "/home/siu856522160/major/test/object_detection/data/data_loader.py", line 781, in partition_data
    #     n_data = len(os.listdir(data_path))
    # FileNotFoundError: [Errno 2] No such file or directory: 'images/train2017'
...
def load_entire_training_data_coco(args, hyp, model):
    save_dir, epochs, batch_size, total_batch_size, weights = (
        Path(args.save_dir),
        args.epochs,
        args.batch_size,
    ...
    # Extract data path and expand user path
    train_path = os.path.join(data_dict["path"], data_dict["train"])
    train_path = os.path.expanduser(train_path)
    print(train_path)

    # Determine number of classes and names
    nc = (1, ["item"]) if args.single_cls else (int(data_dict["nc"]), data_dict["names"])

    # Get grid size from model stride
    gs = int(max(model.stride))

    # Calculate image sizes for training and testing
    imgsz, imgsz_test = [check_img_size(x, gs) for x in args.img_size]
...
def load_entire_test_data_coco(args, hyp, model):
    """
    Loads the entire test data from the COCO dataset.

    Args:
        args: Arguments containing data configuration details.
        hyp: Hyperparameters for model training.
        model: The model to be trained.

    Returns:
        test_data_num: Total number of images in the test dataset.
        test_dataloader_global: Dataloader for the entire test data.
        nc: Number of classes in the dataset (inherited from training data).
    """

    # Load data configuration (assuming same file for both datasets)
    with open(args.data_conf) as f:
        data_dict = yaml.load(f, Loader=yaml.FullLoader)

    # Extract data path and expand user path
    print(data_dict["path"],data_dict["test"])
    test_path = os.path.join(data_dict["path"], data_dict["val"])
    test_path = os.path.expanduser(test_path)
    print(test_path)

    # Reuse previously loaded number of classes (assuming consistent classes)
    nc = args.nc if args.single_cls else int(data_dict["nc"])  # Use defined or loaded nc

    # Get grid size from model stride
    gs = int(max(model.stride))

    # Calculate image sizes for training and testing (potentially different)
    imgsz, imgsz_test = [check_img_size(x, gs) for x in args.img_size]

    # Load test data and create dataloader
    test_dataloader_global, test_dataset_global = create_dataloader(
        test_path,
        imgsz_test,  # Use different image size for testing if needed
        args.batch_size,
        gs,
        args,
        hyp=hyp,
        rect=True,
        augment=False,  # Usually no augmentation for testing
        workers=args.worker_num,
    )

    # Get data size
    test_data_num = test_dataset_global.data_size

    return test_data_num, test_dataloader_global, nc
