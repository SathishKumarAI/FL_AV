...
def load_entire_training_data_coco(args, hyp, model):
    ...
    train_path = os.path.expanduser(os.path.join(data_dict["path"], data_dict["train"]))
    print(train_path)
    ...
    if args.process_id != 0:
        client_idx = int(args.process_id) - 1
        logging.info(f"{client_idx}: client_idx")
        dataloader, dataset = create_dataloader(
            train_path,
            imgsz,
            batch_size,
    ...
    train_data_num = sum(train_data_num_dict.values()) if train_data_num_dict else 0

    return train_data_num, train_data_loader_dict, train_data_num_dict, nc

def load_entire_validation_data_coco(args, hyp, model):
    save_dir = Path(args.save_dir)
    epochs = args.epochs
    batch_size = args.batch_size
    total_batch_size = args.total_batch_size
    ...
    # Extract data path and expand user path
    val_path = os.path.expanduser(os.path.join(data_dict["path"], data_dict["val"]))
    print(val_path)
    ...
    # Calculate image sizes for validation
    imgsz, _ = [check_img_size(x, gs) for x in args.img_size]
    ...
    if args.process_id != 0:
        ...
        dataloader, dataset = create_dataloader(
            ...
            hyp=hyp,
            rect=True,
            augment=False,  # No need to augment validation data
            workers=args.worker_num,
        )
        val_dataset_dict[client_idx] = dataset
        val_data_num_dict[client_idx] = len(dataset)
        val_data_loader_dict[client_idx] = dataloader

    val_data_num = sum(val_data_num_dict.values()) if val_data_num_dict else 0

    return val_data_num, val_data_loader_dict, val_data_num_dict, nc
