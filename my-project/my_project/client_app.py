...
import warnings
import torch
from flwr.common import Code, Status, FitRes
...
class FlowerClient(Client):
    ...
    def __init__(self, model_path: str = "yolov5s.pt"):
        """
        :param model_path: Path to your YOLOv5 .pt weights (e.g., 'yolov5s.pt').
        """
        super().__init__()
        ...
        try:
            ...
            self.model = None  # We'll fail on training/eval if None
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        try:
            ...
            logger.error(f"[Client] Missing 'batch_id' in EvaluateIns.config: {ke}", exc_info=True)
            return EvaluateRes(loss=float("inf"), num_examples=0, metrics={})
        ...
        try:
            ...
            logger.info(
                f"[Client] Local eval done. loss={val_loss}, mAP50={mAP50}, images={num_examples}"
            )
        except Exception as e:
        ...
        metrics = {"mAP50": mAP50}
        return EvaluateRes(loss=float(val_loss), num_examples=int(num_examples), metrics=metrics, status=Status(code=Code.OK, message="Returning fallback after error"),)
...
def client_fn(context: Context):
    ...
    try:
        ...
        model_path = "yolov5s.pt"  # Or dynamically read from context if desired
